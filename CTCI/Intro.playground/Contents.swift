...
func findMinimum(rArray: Array<Int>) -> (min: Int, index: Int) {
    var min = rArray[0]
    var index = 0;
...
var startingPoint = findMinimum(rotatedArray).index
...
func isRansomNoteFromMagazine(ransomNote note: String, magazine: String) -> Bool {
    ...
    let trimmedNote = String(note.characters.filter{extraSeparators.indexOf($0) == nil})
    let trimmedMagazine = String(magazine.characters.filter{extraSeparators.indexOf($0) == nil})
    
    let ransomWords = trimmedNote.componentsSeparatedByString(" ")
    let magazineWords = trimmedMagazine.componentsSeparatedByString(" ")
    ...
    for word in magazineWords {
        if let value = mappedRansomNote[word] {
            if value - 1 == 0 {
                mappedRansomNote.removeValueForKey(word)
            }
        }
    }
...
var result4 = [String]()
func stringPermutation(prefix: String, input: String) {
    if input.characters.count == 0 {
        result4 += [prefix]
        return
    }
    for i in 0..<input.characters.count {
        let currentCharacter = input.startIndex.advancedBy(i)
        let nextCharacter = input.startIndex.advancedBy(i+1)
        stringPermutation(prefix + String(input[currentCharacter]),
            input: input.substringWithRange(Range.init(start: input.startIndex, end: currentCharacter)) + input.substringFromIndex(nextCharacter))
    }
}
