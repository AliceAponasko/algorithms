...
func separateInput(_ string: String) -> (
    min: Int,
    max: Int,
    requirement: Character,
    password: String
) {
    let rangeOfMin = string.firstIndex(of: "-") ?? string.startIndex
    let rangeOfMax = string.firstIndex(of: " ") ?? string.startIndex
    let rangeOfRequirement = string.firstIndex(of: ":") ?? string.startIndex
    let rangeOfPassword = string.lastIndex(of: " ") ?? string.startIndex

    return (
        Int(string[string.startIndex..<rangeOfMin]) ?? 0,
        Int(string[rangeOfMin..<rangeOfMax].dropFirst()) ?? 0,
        Character(String(string[rangeOfMax..<rangeOfRequirement].dropFirst())),
        String(string[rangeOfPassword...].dropFirst()))
}

func validPasswordForOldPolicy(
    min: Int,
    max: Int,
    requirement: Character,
    password: String
) -> Bool {
    let requirementCount = password.filter { $0 == requirement }.count
    return requirementCount >= min && requirementCount <= max
}

func findValidPasswordsForOldPolicy(_ input: [String]) -> Int {
    input
        .filter {
            let separated = separateInput($0)
            return validPasswordForOldPolicy(
                min: separated.min,
                max: separated.max,
                requirement: separated.requirement,
...
func validPasswordForNewPolicy(
    min: Int,
    max: Int,
    requirement: Character,
    password: String
) -> Bool {
    (password[min - 1] == requirement) != (password[max - 1] == requirement)
}

func findValidPasswordsForNewPolicy(_ input: [String]) -> Int {
    input
        .filter {
            let separated = separateInput($0)
            return validPasswordForNewPolicy(
                min: separated.min,
                max: separated.max,
                requirement: separated.requirement,
                password: separated.password
...
print(findValidPasswordsForOldPolicy(input))
print(findValidPasswordsForNewPolicy(input))
