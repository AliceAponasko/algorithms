...
enum Constant {
    static let nextPassport = "\n\n"
    static let passpostSpaceSeparator = " "
    static let passpostNewLineSeparator = "\n"
    static let fieldSeparator = ":"
    static let cmSeparator = "cm"
    static let inSeparator = "in"
    static let poundSeparator: Character = "#"
    static let eyeColors = ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]
    static let hairCharacters = CharacterSet(charactersIn: "abcdef0123456789")
}
...
struct Passport {
    ...
    var isValidByr: Bool { (Int(byr) ?? 0) >= 1920 && (Int(byr) ?? 0) <= 2002 }
    var isValidIyr: Bool { (Int(iyr) ?? 0) >= 2010 && (Int(iyr) ?? 0) <= 2020 }
    var isValidEyr: Bool { (Int(eyr) ?? 0) >= 2020 && (Int(eyr) ?? 0) <= 2030 }
    var isValidEcl: Bool { Constant.eyeColors.contains(ecl) }
    var isValidPid: Bool { pid.count == 9 && CharacterSet.decimalDigits.isSuperset(of: CharacterSet(charactersIn: pid)) }

    var isValidHgt: Bool {
        if let c = Constant.cmSeparator.first,
           let cmIndex = hgt.firstIndex(of: c),
           let cmHeight = Int(hgt.prefix(upTo: cmIndex)),
           hgt.contains(Constant.cmSeparator) {
            return cmHeight >= 150 && cmHeight <= 193
        }
        if let i = Constant.inSeparator.first,
           let inIndex = hgt.firstIndex(of: i),
           let inHeight = Int(hgt.prefix(upTo: inIndex)),
           hgt.contains(Constant.inSeparator) {
    ...
    var isValidHcl: Bool {
        var hairColor = hcl
        let prefix = hairColor.removeFirst()

        return prefix == Constant.poundSeparator &&
            hairColor.count == 6 &&
            hairColor.allSatisfy(
                { CharacterSet(charactersIn: String($0)).isSubset(of: Constant.hairCharacters)}
            )
    }

    init?(_ input: [String: String]) {
        guard let byr = input[CodableKeys.byr],
              let iyr = input[CodableKeys.iyr],
              let eyr = input[CodableKeys.eyr],
              let hgt = input[CodableKeys.hgt],
              let hcl = input[CodableKeys.hcl],
              let ecl = input[CodableKeys.ecl],
              let pid = input[CodableKeys.pid]
        else {
            return nil
        }
        ...
        self.ecl = ecl
        self.pid = pid
        self.cid = input[CodableKeys.cid]
    }
}

func parsePassports(_ input: String) -> [Passport] {
    var passports = [Passport]()
    input
        .components(separatedBy: Constant.nextPassport)
        .forEach {
            if let passport = parsePassport($0), passport.isValid {
...
func parsePassport(_ input: String) -> Passport? {
    let oneStringInput = input.replacingOccurrences(
        of: Constant.passpostNewLineSeparator,
        with: Constant.passpostSpaceSeparator
    )

    var passportMap = [String: String]()

    oneStringInput
        .components(separatedBy: Constant.passpostSpaceSeparator)
        .map {
            let field = $0.components(separatedBy: Constant.fieldSeparator)
            guard let key = field.first,
                  let value = field.last
            else {
                return
            }
...
print(parsePassports(input).count)
