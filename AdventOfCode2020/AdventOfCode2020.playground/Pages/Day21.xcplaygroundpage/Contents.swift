...
struct Food {
    let ingredientList: Set<String>
    let allergens: Set<String>?
}

struct PossibleAllergen {
    let name: String
...
func parse(_ input: String) -> [Food] {
    ...
    input.components(separatedBy: .newlines).forEach { line in
        if line.contains("(") && line.contains(")") {
            ...
            guard let ingredients = components.first,
                  let allergens = components.last?.dropLast()
            else {
                return
            }
            ...
            result.append(
                Food(
                    ...
                    allergens: parseAllergens(String(allergens))
                )
            )
        } else {
            result.append(
                Food(
                    ingredientList: parseIngredients(line),
                    allergens: nil
                )
            )
        }
...
func parseIngredients(_ input: String) -> Set<String> {
    input.components(separatedBy: .whitespaces).reduce(into: Set<String>()) {
        guard !$1.isEmpty else { return }
        $0.insert($1)
    }
}

func parseAllergens(_ input: String) -> Set<String> {
    input.components(separatedBy: .whitespaces).reduce(into: Set<String>()) {
        guard $1 != "contains", !$1.isEmpty else { return }
        $1.contains(",") ? $0.insert(String($1.dropLast())) : $0.insert($1)
    }
}

func matchAllergens(in foods: [Food]) -> [Allergen] {
    var allergens = [Allergen]()

    var possibleAllergens = [PossibleAllergen]()
    var allIngredients = [String: Int]()
    for food in foods {
        food.ingredientList.forEach { allIngredients[$0, default: 0] += 1 }
        food.allergens?.forEach { allergen in
            if let index = possibleAllergens.firstIndex(where: { $0.name == allergen }) {
                possibleAllergens[index] = PossibleAllergen(
                    name: allergen,
                    possibleIngredients: possibleAllergens[index].possibleIngredients.intersection(Set(food.ingredientList))
                )
            } else {
                possibleAllergens.append(PossibleAllergen(name: allergen, possibleIngredients: Set(food.ingredientList)))
            }
        }
    }

    while allergens.count != possibleAllergens.count {
        possibleAllergens
            .filter { $0.possibleIngredients.count == 1 }
            .forEach { possibleAllergen in
                allergens.append(
                    Allergen(
                        name: possibleAllergen.name,
                        ingredient: possibleAllergen.possibleIngredients.first!
                    )
                )

                for i in 0..<possibleAllergens.count {
                    var a = possibleAllergens[i]
                    a.possibleIngredients.remove(possibleAllergen.possibleIngredients.first!)
                    possibleAllergens[i] = a
                }
            }
    }
...
func findNoAllergenIngredients(in foods: [Food], allergens: [Allergen]) -> [String: Int] {
    var allIngredients = [String: Int]()
    for food in foods {
        food.ingredientList.forEach { allIngredients[$0, default: 0] += 1 }
    }
    ...
    return allIngredients.filter { ingredient in
        !allergens.contains(where: { $0.ingredient == ingredient.key })
    }
}

print(findNoAllergenIngredients(
        in: parse(input),
        allergens: matchAllergens(in: parse(input)))
        .map(\.value)
        .reduce(0, +)
)

print(matchAllergens(in: parse(input))
