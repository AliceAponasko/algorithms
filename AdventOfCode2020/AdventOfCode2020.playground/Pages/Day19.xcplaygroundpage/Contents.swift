...
func parseRules(_ input: String) -> [Int: Rule] {
    var currentKey = 0
    var currentRule: Rule = .undefined([[Int]]())
    var currentSubRules = Set<[Int]>()
    var currentSubRuleNumber = [Int]()

    return input.components(separatedBy: "\n\n")
        .first!
        .components(separatedBy: .newlines)
        .reduce(into: [Int: Rule]()) { result, component in
            component.components(separatedBy: .whitespaces)
                .forEach { element in
                    if element.hasSuffix(":") {
                        currentSubRules.insert(currentSubRuleNumber)
                        if case .undefined = currentRule {
                            currentRule = .undefined(Array(currentSubRules))
                        }

                        result[currentKey] = currentRule
                        currentRule = .undefined([[Int]]())
                        currentSubRuleNumber.removeAll()
                        currentSubRules.removeAll()

                        currentKey = Int(element.dropLast())!
                    } else if element == "|" {
                        currentSubRules.insert(currentSubRuleNumber)
                        currentSubRuleNumber.removeAll()
                    } else if element.hasPrefix("\"") && element.hasSuffix("\"") {
                        currentRule = .defined(String(element.dropFirst().dropLast()))
                    } else {
                        currentSubRuleNumber.append(Int(element)!)
                    }
                }
...
func parseMessages(_ input: String) -> [String] {
...
func isValid(message: String, for ruleIndex: Int = 0, allRules: [Int: Rule]) -> [String] {
    let rules = parsedRules[ruleIndex]

    switch rules {
    case let .defined(value):
        return message.hasPrefix(value) ? [String(message.dropFirst(value.count))] : []

    case let .undefined(value):
        return value
            .flatMap { ruleIndexes -> [String] in
                var current = [message]
                ruleIndexes.forEach { rule in
                    current = current.flatMap {
                        isValid(message: $0, for: rule, allRules: allRules)
                    }
                }
                return current
            }
...
var parsedRules = parseRules(input)
//parsedRules[8] = .undefined([[42], [42, 8]])
//parsedRules[11] = .undefined([[42, 31], [42, 11, 31]])
print(parseMessages(input)
        .filter { isValid(message: $0, allRules: parsedRules).contains("") }
        .count
)
